...
def sort_events_by_weekday(events):
  ...
  for event in events:
    ...
    if event.date_weekday() == 6:
      result["sun"] = f"{result['sun']}\n{event_times}\n"
    if event.date_weekday() == 0:
      result["mon"] = f"{result['mon']}\n{event_times}\n"
    if event.date_weekday() == 1:
      result["tue"] = f"{result['tue']}\n{event_times}\n"
    if event.date_weekday() == 2:
      result["wed"] = f"{result['wed']}\n{event_times}\n"
    if event.date_weekday() == 3:
      result["thu"] = f"{result['thu']}\n{event_times}\n"
    if event.date_weekday() == 4:
      result["fri"] = f"{result['fri']}\n{event_times}\n"
    if event.date_weekday() == 5:
      result["sat"] = f"{result['sat']}\n{event_times}\n"
